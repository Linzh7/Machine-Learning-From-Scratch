...
def MyData(rangeStart=1,
    ...
    data_y = np.array(PrebuiltData.Fx(data_x, a, b, c) + randomFloat * random)
...
def MyGivenData():
    ...
    data_y = np.array([
        ...
        33.14, 39.66, 43.24, 43.01, 49.08, 48.67, 51.22, 58.73, 62.35, 63.37,
        66.21, 68.36, 72.58, 77.73, 79.30, 82.41, 84.59, 87.65, 92.71, 94.14,
        98.96, 96.92, 100.49, 103.69, 107.47, 112.30, 112.25, 119.65, 120.27,
        122.58, 126.75, 126.56, 131.66, 135.19, 137.22, 140.52, 144.36, 148.66,
        151.89, 151.59, 154.52, 156.10, 160.59, 167.87, 169.54, 169.42, 175.30,
        176.61, 181.15, 183.19, 187.32, 188.07, 194.08, 192.35, 195.48, 199.27,
        202.93, 208.60, 209.41, 210.97, 216.89, 216.22, 222.08, 226.80, 225.78,
        230.25, 231.79, 234.64, 242.19, 241.19, 243.91, 246.65, 254.63, 254.22,
        260.18, 261.71, 264.59, 267.76, 268.21, 274.39, 275.98, 280.82, 283.98,
...
def MyMultivariateGivenData(a=100, b=200, d=2, e=3, f=1):
    data_x = np.append([range(1, a)], [range(2, b, b // a)]).reshape(2, a - 1)
    data_y = np.array(data_x[0] * d + data_x[1] * e + f * np.random.normal())
    return data_x, data_y
